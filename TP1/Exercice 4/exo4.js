...
const geometry = new THREE.TorusKnotGeometry(4, 1, 100, 8, 3, 2); // TorusKnot
...
gsap.fromTo(mesh.scale, {x:0, y:0, z:0}, {x:1, y:1, z:1, duration: 5}, {ease: "elastic(1, 0.3)"});

window.addEventListener("mousemove", (event) => {
    let r = event.clientX / window.innerWidth;
    let g = event.clientY / window.innerHeight;
    // let b = 1 - event.clientX / window.innerWidth;
    let b = 0.5;
    gsap.to(mesh.material.color, {r: r, g: g, b: b, duration: 2, ease: "elastic"});
 });
